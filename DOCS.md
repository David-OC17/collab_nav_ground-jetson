...
collab_nav_ground-jetson/     ← Jetson Orin Nano workspace
  src/
    ...
    amr_drone_nav/            ← World↔odom TF alignment (alignment_node); map_fusion_node
    fusion/                   ← ICP-based fusion: aerial map + LiDAR occupancy map
    world_mapper/             ← Running ground occupancy map from LiDAR scans
    amr_optitrack/            ← OptiTrack → /amr/pose bridge (standalone teleop tool, not in pipeline)
...
| **AMR Localization** | `ekf_amr`, `aruco_localizer`, `world_mapper`, `amr_optitrack` (standalone) | RDK X3 + Jetson |
| **Navigation & Planning** | `fusion`, `world_mapper`, `trajectory_planner`, `frontier_explorer`, `amr_drone_nav` | Jetson |
...
### 4.4 Step 4 — Map Fusion (`fusion`)

The drone-derived aerial occupancy grid and the AMR's LiDAR-built occupancy grid (generated by `world_mapper` from OraDAR scans) are registered to a common frame using a **coarse search** (correlation or grid-search) followed by **ICP (Iterative Closest Point)** refinement on the extracted obstacle point clouds. The fused grid is an **augmented probabilistic occupancy map** that compensates for individual sensor drift.
...
| `world_mapper` | LiDAR occupancy mapping | [link](src/world_mapper/DOCS.md) |
| `fusion` | Map registration & fusion | [link](src/fusion/DOCS.md) |
...
| `amr_optitrack` | OptiTrack pose bridge (standalone teleop tool) | [link](src/amr_optitrack/DOCS.md) |
| `network_bridge` | Security middleware | [link](network_bridge/DOCS.md) |

> `amr_drone_nav` (TF alignment / `map_fusion_node`) is launched by the orchestrator but does not yet have a per-package `DOCS.md`.
