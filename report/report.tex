...
% Keep floats inside the section that introduces them (prevents figures/tables
% from drifting into later sections, e.g. the security figures).
...
hosts. \TODO{Replace with updated vector (SVG) diagram.}}
\label{fig:sysgraph}
\end{figure*}
...
The software is built on ROS~2 Humble~\cite{ros2}. It spans three main repositories: the Jetson stack
...
The data products exchanged between the major nodes, with their message types,
are summarized in Fig.~\ref{fig:dataflow}.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
  font=\scriptsize, node distance=10mm and 16mm,
  n/.style={draw, rounded corners, fill=blue!5, align=center,
            inner sep=2.5pt, minimum height=5.5mm, text width=1.7cm},
  amr/.style={draw, rounded corners, fill=green!7, align=center,
              inner sep=2.5pt, minimum height=5.5mm, text width=1.7cm},
  sec/.style={draw, dashed, rounded corners, fill=red!8, align=center,
              font=\tiny, inner sep=2pt, text width=1.9cm},
  e/.style={-{Latex[length=1.4mm]}, semithick},
  lbl/.style={font=\tiny, fill=white, inner sep=0.8pt}
]
% aerial nodes (blue, left spine); ground nodes (green, right)
\node[n] (scan) {UAV scan};
\node[n, below=of scan] (build) {map builder};
\node[n, right=of build] (mark) {marker loc.};
\node[n, below=of build] (fuse) {fusion};
\node[amr, below=of mark] (align) {alignment node};
\node[amr, below=of fuse] (plan) {trajectory planner};
\node[amr, below=of align] (wmap) {world\_mapper};
\node[sec, below=7mm of plan] (sec) {\subsys{ros2\_security}\\SecureEnvelope};
\node[amr, below=7mm of sec] (ctrl) {controller};
\draw[e] (scan) -- node[lbl]{\texttt{scan.mp4}} (build);
\draw[e] (scan.east) -| node[lbl,pos=0.25]{video+csv} (mark.north);
\draw[e] (build) -- node[lbl]{\texttt{/drone/map}} (fuse);
\draw[e] (mark) -- node[lbl]{\texttt{/aruco/amr/pose}} (align);
\draw[e] (mark.east) -- ++(7mm,0) |- node[lbl,pos=0.28]{\texttt{/aruco/goal/pose}} (plan.east);
\draw[e] (align) -- node[lbl]{\texttt{world}$\to$\texttt{odom}} (wmap);
\draw[e] (wmap) -- node[lbl]{AMR map} (fuse);
\draw[e] (fuse) -- node[lbl]{\texttt{/fusion/map}} (plan);
\draw[e] (plan) -- node[lbl]{\texttt{/amr/reference}} (sec);
\draw[e] (sec) -- (ctrl);
\draw[e] (ctrl.east) to[bend right=22] node[lbl,pos=0.5]{\texttt{/amr/cmd\_vel}} (wmap.south);
\end{tikzpicture}
\caption{Principal data flow with ROS message types. Aerial products (blue) feed
the planner; ground nodes (green) close the navigation loop. The classifier gate
selects \texttt{/drone/map} or \texttt{/fusion/map} as the planner input
(Sec.~\ref{sec:pipeline}). Inter-host topics are wrapped by \subsys{ros2\_security}
(SecureEnvelope, Sec.~\ref{sec:security}); the planner$\to$controller link is shown
passing through it as a representative case.}
\label{fig:dataflow}
\end{figure}
...
\TODO{Fault-injection test forcing the alternating-ghost mode.}
...
Figure~\ref{fig:stitch-mask} shows the feature-exclusion mask derived from the
blue-tape grid, the resulting masked stitch, the final occupancy product, and the
obstacle transfer projected onto the grid.
...
position to publish \texttt{/aruco/amr/pose} and (on classifier pass)
\texttt{/aruco/goal/pose}. The request carries the absolute paths to the drone
...
and~\ref{fig:vio-ablation}. \TODO{EKF covariance traces.}
...
\subsection{Traceability and Tests}
Satisfies FR-1, FR-4 (emergency-stop actuation), NFR-7.
Figure~\ref{fig:amr-track} shows the AMR tracking a planned reference trajectory.
\TODO{Step-response and tracking-error tests.}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{X_D-AMR_trajectory_following.png}
\caption{AMR tracking a planned reference trajectory under the
feedback-linearizing controller. \TODO{Add quantitative tracking-error
annotation.}}
\label{fig:amr-track}
\end{figure}
...
map under construction during a run. \TODO{Mapping fidelity vs.\ ground-truth
obstacle layout.}
...
\TODO{Replace illustrative values with measured data.}

\begin{table}[t]
\caption{Emergency-Braking Characterization (illustrative figures)}
\label{tab:aeb}
\centering
\footnotesize
...
\begin{figure}[t]
\centering
\includegraphics[height=0.86\textheight]{XV_B-security_pipeline.png}
...
Table~\ref{tab:sec-tests}. \TODO{Replace pass counts and overhead with measured
values.}
...
\begin{table}[t]
\caption{Security Validation Suite (illustrative figures)}
\label{tab:sec-tests}
\centering
\footnotesize
...
\begin{table}[t]
\caption{Per-Message Cryptographic Overhead (illustrative figures)}
\label{tab:sec-overhead}
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Level} & \textbf{Mean} & \textbf{Median} & \textbf{Std} & \textbf{p95} \\
 & (ms) & (ms) & (ms) & (ms) \\
\midrule
\texttt{NONE} (baseline) & 0.04 & 0.04 & 0.01 & 0.06 \\
\texttt{SIGN}            & 0.93 & 0.90 & 0.18 & 1.27 \\
\texttt{SIGN\_ENCRYPT}   & 1.45 & 1.41 & 0.24 & 1.92 \\
\bottomrule
\end{tabular}
\end{table}

At \texttt{SIGN\_ENCRYPT}, the added ${\sim}\SI{1.4}{\milli\second}$ median
per-message latency (Table~\ref{tab:sec-overhead}) is small relative to the slowest mission-critical topic period
(the \SI{15}{\hertz} LiDAR, \SI{66.7}{\milli\second}), satisfying the real-time
budget (NFR-11).
...
Experiments were run in the $4\times4$~m arena of Fig.~\ref{fig:arena}
(Sec.~\ref{sec:intro}), with cardboard-box obstacles and floor-mounted ArUco
markers; OptiTrack provides millimetre-accurate ground truth for both robots.
\TODO{Obstacle configurations, marker placement, number of runs.}
