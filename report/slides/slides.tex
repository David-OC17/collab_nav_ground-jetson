...
odometry, \alert{85\,\% mission success}, and a fully ROS\,2 implementation across
...
\column{0.44\textwidth}
\centering
\includegraphics[height=0.66\textheight]{VII_D-random_forest_feature_importance.png}\\[0.2em]
...
\vspace{-0.2em}
\centering
\includegraphics[width=0.7\linewidth]{IX_D-vio_gate_ablation.png}\\
...
\column{0.48\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{IX_D-amr_trajectory_tracking-new.png}\\[0.2em]
...
\column{0.48\textwidth}
\centering
\includegraphics[width=0.68\linewidth]{XI_D-mapping_progress.png}\\[0.2em]
...
\textbf{Aruco based Homing} The final approach detects arucos in the goal cube and uses OpenCV PnP solver to translate their poses into the global map.
Then A* is used to navigate to the goal position. 
\end{columns}
\end{frame}
...
\column{0.48\textwidth}
\centering
\includegraphics[width=0.86\linewidth]{XV_D-round_trip_latency.png}\\[0.3em]
\includegraphics[width=0.86\linewidth]{XV_D-crypto_latency_breakdown.png}
\end{columns}
\end{frame}
...
\end{block}
\begin{block}{Navigation ($N=20$)}
Goal reached in \textbf{17/20 (85\,\%)} with \emph{no collisions}; mean cross-track
\SI{6.1}{\centi\meter}, mean mission \SI{74}{\second}. The 3 failures stalled on
early-mission map sparsity; autonomous replanning handled all other blockages.
\end{block}
\column{0.5\textwidth}
...
$\sim$88\,\% lower position error \;\textbar\; 85\,\% mission success, no collisions
